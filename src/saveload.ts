...
export function mergeObj_nocopy(
  ...
  obj_from: { [key: string | number | symbol]: any }
): any {
  ...
  Object.keys(obj_from).forEach((key) => {
    ...
    } else if (typeof obj_to[key] === 'object' || typeof obj_from[key] === 'object')
      rslt[key] = mergeObj_nocopy(obj_to[key], obj_from[key]);
  });
  return rslt;
}
